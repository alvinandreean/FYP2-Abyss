...
class FGSM:
    ...
    def display_attack_results(self, original_image, perturbation, adversarial_image, 
        ...
        # Different models use different preprocessing, handle each correctly for display
        if self.model_name == 'vgg19':
            # VGG19 preprocessing subtracts the ImageNet mean from each color channel
            # RGB mean values: [123.68, 116.779, 103.939]
            rgb_means = np.array([123.68, 116.779, 103.939])
            
            # Denormalize by adding back the means
            display_original = np.clip((original_image[0] + rgb_means) / 255.0, 0, 1)
            display_adv = np.clip((adversarial_image[0] + rgb_means) / 255.0, 0, 1)
            
            # For perturbation, use standard normalization for better visibility
            display_pert = np.clip(perturbation[0] * 0.5 + 0.5, 0, 1)
        else:
            # For MobileNetV2, InceptionV3, and DenseNet which use [-1,1] normalization
